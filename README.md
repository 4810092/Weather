...
- The coordinated `1.1.0` source identities are Android phone 8, Wear 1000008,
  ...
  source `6f72e70` has no retained signed candidate bytes. Historical QA
  does not establish signing or physical coverage for the current candidates.
  Current product/build source `6f72e70` keeps the Apple source-revision and
  ...
  Compose UI matrix. That
  matrix has not produced a hosted result yet; prior CI, unsigned, and device
  evidence is non-transferable.
  See the [growth
  implementation checkpoint](docs/GROWTH_RELEASE.md), the
  [source-sync gate](growth/quality/release-artifact-source-sync-2026-08-30-6f72e70.md),
