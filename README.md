...
- The coordinated `1.1.0` candidate is not uploaded or public. Signed Android phone/Wear artifacts and the Apple app/widget/watch IPA are locally validated; the signed phone APK passed bounded physical API 25 clean/live/cold-start. Accessibility/share evidence belongs to the preceding debug artifact. See the [growth implementation checkpoint](docs/GROWTH_RELEASE.md) and the historical [release candidate record](docs/RELEASE_CANDIDATE.md).
