...
source `fb591e3` keeps fail-closed Apple source-revision plumbing, assigns
...
phone and tablet jobs plus the existing Android/iOS jobs. Predecessor run
`33295070238` passed ordinary Android/iOS but failed two phone visibility
assertions and the tablet KVM permission gate. The current source corrects
those exact failures; its hosted rerun is pending. A protected, manual GitHub-hosted workflow
...
regression evidence and do not transfer to `fb591e3`. None closes signing,
...
| Current release source authority | [quality/release-artifact-source-sync-2026-08-30-fb591e3.md](quality/release-artifact-source-sync-2026-08-30-fb591e3.md) | Exact `fb591e3` product/build identity, predecessor failed-run diagnosis, hosted API 24/API 36 rerun boundary, non-upload export contract, and explicit non-transferability of every earlier artifact/device result |
