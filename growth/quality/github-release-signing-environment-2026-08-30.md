...
Status: **CREATED AND BRANCH-RESTRICTED; 0/8 REQUIRED SECRETS
...
## Empty-secret boundary

The environment currently contains no secrets and no variables. None of the
eight workflow inputs below was uploaded, printed, copied into the repository,
...
Creating the environment closes only the repository-configuration prerequisite.
It does not prove credential access, signing, artifact identity, physical QA,
store upload, submission, review, rollout, or public availability. The manual
...
login Keychain, followed by direct provisioning of the eight values into this
environment without exposing their contents. Only then may the master-only
hosted workflow be dispatched.
